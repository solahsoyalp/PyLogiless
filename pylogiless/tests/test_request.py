...
requests.request を unittest.mock でモックし、ネットワークアクセスをせずに
...
class TestLogilessClientRequest:
    ...
    def test_returns_dict_on_json_response(self):
        ...
        with mock.patch("requests.request", return_value=_make_response(200, expected)):
    ...
    def test_400_raises_validation_error(self):
        ...
        with mock.patch("requests.request", return_value=_make_response(400, body)):
    ...
    def test_401_raises_auth_error(self):
        ...
        """
        with mock.patch("requests.request", return_value=_make_response(401, {"error": "unauthorized"})):
    ...
    def test_403_raises_auth_error(self):
        ...
        """
        with mock.patch("requests.request", return_value=_make_response(403, {"error": "forbidden"})):
            with pytest.raises(LogilessAuthError):
                self.client.request("GET", self.url)
    ...
    def test_423_raises_resource_locked_error(self):
        ...
        """
        with mock.patch("requests.request", return_value=_make_response(423, {"error": "locked"})):
            with pytest.raises(LogilessResourceLockedError):
                self.client.request("GET", self.url)

    def test_429_raises_rate_limit_error(self):
        """
        429 で LogilessRateLimitError が送出されることを検証
        """
        with mock.patch("requests.request", return_value=_make_response(429, {"error": "too many"})):
            with pytest.raises(LogilessRateLimitError):
                self.client.request("GET", self.url)

    def test_500_raises_server_error(self):
        """
        500 で LogilessServerError が送出されることを検証
        """
        with mock.patch("requests.request", return_value=_make_response(500, {"error": "server"})):
    ...
    def test_request_exception_wrapped_in_logiless_error(self):
        ...
        """
        with mock.patch("requests.request", side_effect=RequestException("接続失敗")):
    ...
    def test_default_headers_contain_auth_and_content_type(self):
        ...
        """
        with mock.patch("requests.request", return_value=_make_response(200, {})) as mocked:
            self.client.request("GET", self.url)
    ...
    def test_params_and_json_passed_to_requests(self):
        """
        params / json が requests.request にそのまま渡されることを検証
        """
